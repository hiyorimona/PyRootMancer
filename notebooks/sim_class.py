...
class Simulation:
    ...
    def create_robots(self, num_agents):
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if (
                ...
                ):  # Check if more agents need to be placed
                    ...
                    planeId = p.loadURDF(
                        "custom.urdf", position_with_offset, rotate_90
                    )  # start_orientation)
                    # Disable collision between the robot and the specimen
                    p.setCollisionFilterPair(
                        robotId, planeId, -1, -1, enableCollision=0
                    )
                    spec_position, spec_orientation = (
                        p.getBasePositionAndOrientation(planeId)
                    )
                    ...
                    p.changeVisualShape(
                        planeId, -1, textureUniqueId=self.textureId
                    )
                    ...
                    self.pipette_positions[f'robotId_{robotId}'] = (
                        pipette_position
                    )
    ...
    def drop(self, robotId):
        ...
        specimen_position = p.getBasePositionAndOrientation(
            self.specimenIds[0]
        )[0]
        ...
        collision = p.createCollisionShape(
            shapeType=p.GEOM_SPHERE, radius=sphereRadius
        )
        ...
        p.resetBasePositionAndOrientation(
            sphereBody, droplet_position, [0, 0, 0, 1]
        )
        # track the sphere id
    ...
    def check_contact(self, robotId, specimenId):
        for sphereId in self.sphereIds:
            ...
            if contact_points_specimen:
                ...
                sphere_position, sphere_orientation = (
